; Sorry Evl, but the player you send me still does a reset of the internal
; IDE, mine does not; please, spread my own assembled version of the
; player or include this message at line 183.
;		DC.B " Falcon-fix by FroST "
; So people wouldn't say I'm the biggest lamer of the world.
; Ok, 505 tells me to not spread it, but I hope he wouldn't be angry
; after me.
; And don't say you've done the same modifications, I've traced the player
; you gave me, reg 7 of the YM is still not good accessed.
; Now go to line 412 to see what I do.
; Just a question : Do you have an IDE drive or a SCSI drive on your
; Falcon.

;; Falcon-IDE-fix by FroST, see line 412.

;Animal Mine SidSoundDesigner Replayer         DONT spReaD!
;
; reassembled,fixed and new stuff added  by .defjam./.checkpoint.
;
;  ??? = Unklar

;;    set_music_off   ;+12          Stop Replay & Resets MFP Vectors

;;    set_music_on    ;+16          Start Replay at last pos.

;; CF LIGNE 420 pour les modifs par FroST.

; set_sid_on/off ... totally needles!


; I'm in a hurry, hope you can handle dis ...
...
; +12 music OFF
; +16 music ON
; +28 select replay hz (d0=frq)

; sc68init:	
;                 lea     vset1,A0
;                 lea     sound1,A1

music:                                  ;      IBYTES 'C:\SIDRAVE3.DAT'
                bra     init_music      ;+0

                bra     my_replay_music ;;   store_regs_replay_music ;+4
...
lall            DC.B "SSD-Player by Animal Mine! "
		...
		DC.B " Falcon-fix by FroST "	; Do not remove please
                EVEN
;                ENDPART
...
save_MFP:       ;>PART
                ...
                bclr    #5,$FFFFFA09.w  ;Timer C
                bclr    #4,$FFFFFA09.w  ;Timer D
                bclr    #5,$FFFFFA13.w  ;Timer A
                bclr    #5,$FFFFFA15.w  ;Timer C
                bclr    #4,$FFFFFA15.w  ;Timer D

                lea     irq_vectors(PC),A0
                move.l  $00000110.w,(A0)+ ;Timer C
                move.l  $00000114.w,(A0)+ ;Timer D
...
;Stop All Timers
                ...
                lea     irq_vectors(PC),A0
                move.l  (A0)+,$00000110.w ;Timer C
                move.l  (A0)+,$00000114.w ;Timer D
...
set_sid_on:    ; >PART
                ...
                move.l  #tc0,$00000114.w ;Timer C
                andi.b  #$0F,$FFFFFA1D.w
                ori.b   #%00111000,$FFFFFA1D.w
                move.b  #255,$FFFFFA23.w ;data
                bset    #5,$FFFFFA15.w
                bclr    #5,$FFFFFA09.w  ;
...
set_sid_off:   ; >PART
                ...
                move.l  A0,$00000114.w  ;Timer C
                clr.b   $FFFFFA1D.w
                move.b  #255,$FFFFFA23.w
                bclr    #5,$FFFFFA15.w
                bclr    #5,$FFFFFA09.w  ;
...
copy_sid_psg_rout:  ; >PART         ;SID PSG Volume Routs to $200
...
clear_YM:      ; >PART	**; Cette routine fait un reset du disque j'ai
			;*; l'impression
; the old stuff that resets the internal IDE
;                moveq   #$0A,D0	; 10 -> 0
;clr_ym_r:       move.b  D0,$FFFF8800.w
;                clr.b   $FFFF8802.w
;                dbra    D0,clr_ym_r

; This works well but is a bit heavy
	move.b	#$a,$ffff8800.w
	clr.b	$ffff8802.w
	move.b	#9,$ffff8800.w
	clr.b	$ffff8802.w
...
;               move    #$2400,SR  --> IRQS FREIGEBEN!  (Timer B !!!)
sid_irqs_s:
;               move    #$2400,SR
                move.l  #$08080000,$FFFF8800.w
                move.w  #tc1,$00000116.w
                rte
	
;               move    #$2400,SR
                move.l  #$08080000,$FFFF8800.w ;<--
                move.w  #tc0,$00000116.w
                rte
...
ActivSidVoiceA: ; move.l  #L0043,$00000120.w
                move.l  #tc0,$00000114.w
                st      SidOnOffA
                rts
	
ActivSidVoiceB: move.l  #ta0,$00000134.w
                st      SidOnOffB
                rts
	
ActivSidVoiceC: move.l  #td0,$00000110.w
                st      SidOnOffC
                rts

StopSidVoiceA: ;  move.l  #VoidIrq,$00000120.w
                move.l  #VoidIrq,$00000114.w
                sf      SidOnOffA
                rts
	
StopSidVoiceB:	move.l  #VoidIrq,$00000134.w
                sf      SidOnOffB
                rts

StopSidVoiceC:  move.l  #VoidIrq,$00000110.w
                sf      SidOnOffC
                rts
...
;               lea     start_player_bss(PC),A4
;               move.w  #(end_player_bss-start_player_bss)-1,D7
;  clr_b:                                  *;     clr.b   (A4)+
;               dbra    D7,clr_b
...
.okInfo:        moveq   #$1F,D0			; $$$ ben: overflow !!!
.cpyInfo:       move.l  (A0)+,(A1)+
                move.l  (A0)+,(A1)+
                dbra    D0,.cpyInfo
...
L0096:          move.l  #tc0,$00000114.w ;Timer C
                andi.b  #$0F,$FFFFFA1D.w
                ori.b   #%00111000,$FFFFFA1D.w
                clr.b   $FFFFFA23.w     ;Timer C data
                bset    #5,$FFFFFA15.w
                bclr    #5,$FFFFFA09.w  ;
